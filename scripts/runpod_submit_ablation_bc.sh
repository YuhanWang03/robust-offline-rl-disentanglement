...
# ============================================================
# RunPod: Ablation A — Behavioral Cloning for all encoder methods.
# Mirrors submit_ablation_bc.sh but runs directly on the pod
# without Slurm / sbatch.
#
...
NOTEBOOK_DIR="$PROJECT_ROOT/notebooks/ablation_bc"
...
echo
echo "--- Running true_only_bc ---"
for seed in "${SEEDS[@]}"; do
  run_true_only_job "$seed"
done

echo
echo "[$(timestamp)] All bc ablation jobs finished for ENV_NAME=$ENV_NAME"
