...
# ------------------------------------------------------------
# Ablation A (algorithm): Behavioral Cloning for all encoder methods.
...
SLURM_TIME="${SLURM_TIME:-02:00:00}"   # BC trains faster than IQL/TD3+BC
...
TRUE_ONLY_NB="exp_true_only_bc.ipynb"
...
submit_noisy_job() {
  ...
  local notebook_path="$NOTEBOOK_DIR/ablation_bc/$notebook"
...
submit_true_only_job() {
  ...
  local job_script="$JOB_DIR/${job_name}.slurm"
  local notebook_path="$NOTEBOOK_DIR/ablation_bc/$TRUE_ONLY_NB"
...
# ------------------------------------------------------------
echo
echo "--- Submitting true_only_bc ---"
for seed in "${SEEDS[@]}"; do
  submit_true_only_job "$seed"
done

echo
echo "All bc ablation jobs submitted for environment: $ENV_NAME"
