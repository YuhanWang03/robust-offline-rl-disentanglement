...
# ------------------------------------------------------------
# External comparison methods from literature:
#   - exp_pca_iql.ipynb       PCA-IQL: linear encoder, no privileged info
#   - exp_raw_noisy_riql.ipynb RIQL: no encoder, robust IQL algorithm
#   - exp_denoised_mdp.ipynb  Denoised MDP: neural encoder, no privileged info
#
# Each notebook is submitted as an independent Slurm job over
# the full noise grid (seeds × dims × scales × types).
#
...
SLURM_TIME="${SLURM_TIME:-04:00:00}"   # Denoised MDP pretraining dominates; PCA/RIQL finish well within this
...
submit_job() {
  ...
  local notebook_path="$NOTEBOOK_DIR/external_methods/$notebook"
...
echo
echo "All external method jobs submitted for environment: $ENV_NAME"
