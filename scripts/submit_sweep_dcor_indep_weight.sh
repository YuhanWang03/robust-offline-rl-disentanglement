...
# ------------------------------------------------------------
# Sweep: dCor indep_weight on Ant nonlinear noise
# Fix (env, noise_dim, noise_scale, noise_type, seed), vary INDEP_WEIGHT
# dCor is normalized to [0,1]; high-dim curse reduces signal — sweep biased toward larger values
# ------------------------------------------------------------
...
INDEP_WEIGHTS=(
  0.1 0.3 0.5 1.0 2.0 3.0 5.0 8.0
...
NOTEBOOK_PATH="$PROJECT_ROOT/notebooks/ablation_indep_weight/exp_disentangled_dcor_indep_sweep.ipynb"
...
for iw in "${INDEP_WEIGHTS[@]}"; do
  ...
  job_name="dcor_sweep_${ENV_NAME}_nd${NOISE_DIM}_ns${scale_tag}_s${SEED}_${iw_tag}"
...
echo
echo "Submitted ${#INDEP_WEIGHTS[@]} jobs for dCor indep_weight sweep."
